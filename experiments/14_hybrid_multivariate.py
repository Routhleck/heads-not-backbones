"""
Experiment 14: HybridPAB multivariate walk-forward + DM test.
...
Output:
- results/14_hybrid_multivariate.json: per-fold metrics + DM stats
- results/14_hybrid_multivariate_summary.md: human-readable summary
- figures/14_hybrid_multivariate_per_fold.png: per-fold MAE line plot
"""
import sys
import os
import json
...
def train_and_predict(args):
    ...
    model_name, model_spec, horizon, fold_idx, seed, X_train, Y_train, X_test, Y_test, n_channels, output_ch = args
    ...
    if model_spec == "linear":
        ...
        if model_cls is PeriodAttentionTimesNetLite:
            ...
            model.fit_periods(X_train, max_period=60, min_period=4)  # multi-channel AMPD
            model = model.to(DEVICE)
        elif model_cls is TimesNetMV:
...
def main():
    print("=" * 70)
    print(" Experiment 14: HybridPAB multivariate walk-forward (5 channels)")
    print("=" * 70)
    ...
    for h in HORIZONS:
        for fold in folds_by_h[h]:
            for name, (model_spec, fixed_ch) in MODELS.items():
                ...
                # Determine output channels — HybridPAB(5ch) predicts only target channel
                output_ch = 1 if name == "HybridPAB(5ch)" else (1 if fixed_ch == 1 else N_CHANNELS)
                # Y is sliced to match output channels (target-only for output_ch=1)
                if output_ch == 1:
                ...
                for seed in (SEEDS if model_spec != "linear" else [0]):
                    tasks.append((
                        ...
                        X_train, Y_train, X_test, Y_test, n_ch, output_ch,
                    ))
    ...
    # DM tests: HybridPAB(5ch) vs each baseline
    baselines_to_test = ["Linear", "DLinear", "iTransformer", "TimesNet", "PAB(1ch)", "PAB(5ch)"]
    ...
    for h in HORIZONS:
        ...
        for baseline in baselines_to_test:
            ...
            for fold_idx in range(len(folds_by_h[h])):
                pab_key = ("HybridPAB(5ch)", h, fold_idx)
    ...
    out_path = RESULTS_DIR / "14_hybrid_multivariate.json"
    ...
    print("=" * 70)
    model_order = ["Linear", "DLinear", "iTransformer", "TimesNet", "PAB(1ch)", "PAB(5ch)", "HybridPAB(5ch)"]
    header = f"{'Horizon':<8}"
    ...
    print("\n" + "=" * 70)
    print(" Diebold-Mariano: HybridPAB(5ch) vs baseline (negative DM = HybridPAB better)")
    print("=" * 70)
    for h in HORIZONS:
        print(f"\n  h={h}:")
    ...
    try:
        ...
        colors = {'Linear':'#999', 'DLinear':'#888', 'iTransformer':'#555',
                  'TimesNet':'#1f77b4', 'PAB(1ch)':'#ff7f0e', 'PAB(5ch)':'#d62728', 'HybridPAB(5ch)':'#9467bd'}
        markers = {'Linear':'o', 'DLinear':'s', 'iTransformer':'D',
                  'TimesNet':'X', 'PAB(1ch)':'^', 'PAB(5ch)':'*', 'HybridPAB(5ch)':'d'}
        ...
        plt.suptitle("Phase 5 — HybridPAB multivariate walk-forward MAE per fold", y=1.02)
        plt.tight_layout()
        fig_path = FIGURES_DIR / "14_hybrid_multivariate_per_fold.png"
    ...
    md.append("")
    md.append("## Diebold-Mariano: HybridPAB(5ch) vs each baseline")
    md.append("")
    md.append("Negative DM stat = HybridPAB(5ch) better. p-values from Student-t (df=n-1).")
    md.append("")
    for h in HORIZONS:
        md.append(f"### h={h}")
        md.append("")
        ...
        md.append("")
    md_path = RESULTS_DIR / "14_hybrid_multivariate_summary.md"
    md_path.write_text("\n".join(md))
